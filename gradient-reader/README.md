...
- **It looks wrong on a dark site.** It should switch to Night by itself. If you
  have deliberately chosen a palette, that choice is kept and not overridden.
...
| `palettes.js` | The five palettes. Data only, with the colour-vision reasoning. |
...
| `test/check-engine.js` | `node test/check-engine.js` — 3,300 checks on the arithmetic. |
